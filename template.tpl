___TERMS_OF_SERVICE___

By creating or modifying this file you agree to Google Tag Manager's Community
Template Gallery Developer Terms of Service available at
https://developers.google.com/tag-manager/gallery-tos (or such other URL as
Google may provide), as modified from time to time.


___INFO___

{
  "type": "TAG",
  "id": "cvt_temp_public_id",
  "version": 1,
  "securityGroups": [],
  "displayName": "SafeOpt - Conversion Tag",
  "categories": [
    "TAG_MANAGEMENT"
  ],
  "brand": {
    "id": "brand_safeopt",
    "displayName": "SafeOpt",
    "thumbnail": "data:image/png;base64,\u003d\u003d"
  },
  "description": "This tag will track conversions and suppress converted users from receiving campaign emails. This tag should trigger on a checkout success page (e.g. thank you, order confirmation, etc).",
  "containerContexts": [
    "WEB"
  ]
}


___TEMPLATE_PARAMETERS___

[
  {
    "type": "SELECT",
    "name": "value",
    "displayName": "Value",
    "macrosInSelect": true,
    "selectItems": [],
    "simpleValueType": true,
    "help": "Subtotal value for the order (before tax and shipping)"
  },
  {
    "type": "SELECT",
    "name": "order_id",
    "displayName": "Order ID",
    "macrosInSelect": true,
    "selectItems": [],
    "simpleValueType": true,
    "help": "Unique ID for order"
  },
  {
    "type": "SELECT",
    "name": "email",
    "displayName": "User Email",
    "macrosInSelect": true,
    "selectItems": [],
    "simpleValueType": true,
    "notSetText": "User Email Not Available"
  },
  {
    "type": "SELECT",
    "name": "currency",
    "displayName": "Currency",
    "macrosInSelect": true,
    "selectItems": [],
    "simpleValueType": true,
    "notSetText": "Hard Code Currency",
    "subParams": [
      {
        "type": "TEXT",
        "name": "currency_hard_code",
        "displayName": "Currency Hard Coded",
        "simpleValueType": true,
        "help": "Hard code currency if currency is not available as a variable.  Must use ISO 4217 currency codes (e.g. USD)",
        "valueValidators": [
          {
            "type": "STRING_LENGTH",
            "args": [
              3,
              3
            ]
          },
          {
            "type": "NON_EMPTY"
          }
        ],
        "enablingConditions": [
          {
            "paramName": "currency",
            "paramValue": "",
            "type": "NOT_PRESENT"
          }
        ],
        "defaultValue": "USD"
      }
    ],
    "help": "If your store only processes USD then select Hard Code Currency."
  },
  {
    "type": "SELECT",
    "name": "offer_code",
    "displayName": "Offer Code",
    "macrosInSelect": true,
    "selectItems": [],
    "simpleValueType": true,
    "notSetText": "Offer Code Not Available",
    "help": "Offer code used by user when placing the order"
  }
]


___SANDBOXED_JS_FOR_WEB_TEMPLATE___

// SafeOpt Converstion Tag Code
const log = require('logToConsole');
const setInWindow = require('setInWindow');

log('data =', data);

var currency_main = data.currency_hard_code ? data.currency_hard_code : data.currency;

var conversion_obj = {
        order_id: data.order_id, 
        value: data.value, 
        currency: currency_main,
        email: data.email,
        offer_code: data.offer_code
    };


setInWindow('AddShoppersConversion', conversion_obj);

// Call data.gtmOnSuccess when the tag is finished.
data.gtmOnSuccess();


___WEB_PERMISSIONS___

[
  {
    "instance": {
      "key": {
        "publicId": "logging",
        "versionId": "1"
      },
      "param": [
        {
          "key": "environments",
          "value": {
            "type": 1,
            "string": "debug"
          }
        }
      ]
    },
    "clientAnnotations": {
      "isEditedByUser": true
    },
    "isRequired": true
  },
  {
    "instance": {
      "key": {
        "publicId": "access_globals",
        "versionId": "1"
      },
      "param": [
        {
          "key": "keys",
          "value": {
            "type": 2,
            "listItem": [
              {
                "type": 3,
                "mapKey": [
                  {
                    "type": 1,
                    "string": "key"
                  },
                  {
                    "type": 1,
                    "string": "read"
                  },
                  {
                    "type": 1,
                    "string": "write"
                  },
                  {
                    "type": 1,
                    "string": "execute"
                  }
                ],
                "mapValue": [
                  {
                    "type": 1,
                    "string": "AddShoppersConversion"
                  },
                  {
                    "type": 8,
                    "boolean": true
                  },
                  {
                    "type": 8,
                    "boolean": true
                  },
                  {
                    "type": 8,
                    "boolean": false
                  }
                ]
              }
            ]
          }
        }
      ]
    },
    "clientAnnotations": {
      "isEditedByUser": true
    },
    "isRequired": true
  }
]


___TESTS___

scenarios: []


___NOTES___

Created on 5/30/2023, 12:06:15 PM


